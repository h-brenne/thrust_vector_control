...
class ThrustVectorController : public Controller {
    public:
        ThrustVectorController(float velocity, float amplitude, float phase, float experiment_length, float startup_sequence_length = 1.0);
        ...
        void constant_command_run(MoteusInterface::ServoCommand* command, float elapsed_seconds);
        void startup_sequence_run(MoteusInterface::ServoCommand* command, float elapsed_seconds);
