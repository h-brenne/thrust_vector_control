...
def downsample(force_vectors, timestamps, sequence_length, experiment_duration, transient_duration):
...
def find_change_indices(amplitude_commands, velocity_commands, phase_commands):
    ...
    for i in range(1, len(amplitude_commands)):
        ...
        if (
            current_amplitude != previous_amplitude or
            current_velocity != previous_velocity or
...
def find_unique_command_data(command_data):
    ...
    change_indices = find_change_indices(amplitude_commands, velocity_commands, phase_commands)
    ...
    sequence_duration = len(unique_command_timestamps)*np.diff(unique_command_timestamps)[1]
    ...
    return unique_command_timestamps, sequence_duration, sequence_length, \
        amplitude_commands, phase_commands, velocity_commands
...
def write_time_offset_to_header(force_file, force_time_offset):
    ...
    if "Force Time Offset" not in header:
        ...
        header = re.sub(r"Force Time Offset = [\d.]+", f"Force Time Offset = {force_time_offset:.2f}", header)
...
def process_dataset(command_file, force_file, startup_time, transient_duration):
    ...
    unique_command_timestamps, sequence_duration, sequence_length, \
    amplitude_commands, phase_commands, velocity_commands = find_unique_command_data(df)
    # Load force data
    ...
    force_df['seconds'] = np.linspace(-force_offset, num_samples / 2000 - force_offset, num_samples)
    # Filter force data
    force_df_masked = force_df[(force_df["seconds"] >= 0) & (force_df["seconds"] <= sequence_duration)]
    force_df = force_df_masked.ewm(span=500).mean()
    force_vectors = np.stack((force_df_masked["Force X (N)"], force_df_masked["Force Y (N)"], force_df_masked["Force Z (N)"]), axis=1)

    # Downsample force_vectors using the computed sequence_duration and sequence_length
    force_vectors_downsampled, force_timesteps_downsampled = downsample(force_vectors, force_df_masked['seconds'], sequence_length, sequence_duration, transient_duration)

    # Calculate elevation and azimuth angles
    elevation_angles, azimuth_angles = calc_elevation_azimuth_angles(force_vectors_downsampled)
    force_magnitudes = np.linalg.norm(force_vectors_downsampled, axis=1)

    return amplitude_commands, phase_commands, velocity_commands, elevation_angles, azimuth_angles, force_magnitudes, \
           command_df, force_df, unique_command_timestamps, force_timesteps_downsampled