...
from analyze_thrust_vectoring import process_dataset, multiple_linear_regression, \
    custom_linear_model, quadratic_model, exponential_model
...
for idx, (command_file, force_file) in enumerate(datasets):
    amplitude_commands, phase_commands, velocity_commands, elevation_angles, azimuth_angles, force_magnitudes, \
    _, force_df_smooth, unique_command_timestamps, force_timesteps_downsampled = process_dataset(command_file, force_file, startup_time, transient_duration)
...
mag_slope, mag_slope2, mag_intercept = multiple_linear_regression(combined_velocity_commands, combined_amplitude_commands, combined_force_magnitudes)
amp_slope, _ = curve_fit(custom_linear_model, combined_amplitude_commands[non_zero_amplitude_indexes], combined_elevation_angles[non_zero_amplitude_indexes])
quad_coeffs, _ = curve_fit(quadratic_model, combined_velocity_commands[high_amplitude_indexes], combined_azimuth_angles[high_amplitude_indexes])
exponential_coeffs, _ = curve_fit(exponential_model, combined_velocity_commands, combined_force_magnitudes)
### Plotting ###
...
plt.figure()
plt.scatter(combined_amplitude_commands[non_zero_amplitude_indexes], combined_elevation_angles[non_zero_amplitude_indexes], c=combined_velocity_commands[non_zero_amplitude_indexes], cmap='viridis')
...
x_vel = np.linspace(min(combined_velocity_commands), max(combined_velocity_commands), 100)
y_phase = quadratic_model(x_vel, *quad_coeffs)
plt.figure()
plt.scatter(combined_velocity_commands[high_amplitude_indexes], combined_azimuth_angles[high_amplitude_indexes], c=combined_amplitude_commands[high_amplitude_indexes], cmap='viridis')
...
ax.scatter(combined_velocity_commands, combined_amplitude_commands, combined_force_magnitudes, c='blue', label='Actual Data')

# Create a grid of VelocityCommand and AmplitudeCommand values
x_range = np.linspace(min(combined_velocity_commands), max(combined_velocity_commands), 100)
y_range = np.linspace(min(combined_amplitude_commands), max(combined_amplitude_commands), 100)
...
ax.legend()
plt.title('Force Magnitude vs Velocity Command and Amplitude Command (Multiple Linear Regression)')
