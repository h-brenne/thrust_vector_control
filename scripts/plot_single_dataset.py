...
from analyze_thrust_vectoring import process_dataset, multiple_linear_regression, \
    custom_linear_model, quadratic_model
...
amplitude_commands, phase_commands, velocity_commands, elevation_angles, azimuth_angles, force_magnitudes, \
    command_df, force_df, unique_command_timestamps, force_timesteps_downsampled = process_dataset(command_file, force_file, startup_time, transient_duration)

# Print time duration of force_df
### Thrust vector model ###
...
print("force time duration: ", force_df['seconds'].iloc[-1] - force_df['seconds'].iloc[0])
print("command time duration: ", command_df['seconds'].iloc[-1] - command_df['seconds'].iloc[0])

mag_slope, mag_slope2, mag_intercept = multiple_linear_regression(velocity_commands, amplitude_commands, force_magnitudes)
amp_slope, _ = curve_fit(custom_linear_model, amplitude_commands[non_zero_amplitude_indexes], elevation_angles[non_zero_amplitude_indexes])
quad_coeffs, _ = curve_fit(quadratic_model, velocity_commands[high_amplitude_indexes], azimuth_angles[high_amplitude_indexes])
...
plt.figure()
plt.scatter(amplitude_commands[non_zero_amplitude_indexes], elevation_angles[non_zero_amplitude_indexes], c=velocity_commands[non_zero_amplitude_indexes], cmap='viridis')
...
plt.figure()
plt.scatter(velocity_commands[high_amplitude_indexes], azimuth_angles[high_amplitude_indexes], c=amplitude_commands[high_amplitude_indexes], cmap='viridis')
...
ax.scatter(velocity_commands, amplitude_commands, force_magnitudes, c='blue', label='Actual Data')
...
plt.figure()
plt.subplot(2, 1, 1)
force_magnitude_full = np.linalg.norm(force_df[["Force X (N)", "Force Y (N)", "Force Z (N)"]], axis=1)
