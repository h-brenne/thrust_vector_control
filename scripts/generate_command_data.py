...
def generate_thrust_vector_sequence(min_velocity, max_velocity, step_velocity,
                                    min_amplitude, max_amplitude, step_amplitude,
                                    min_phase, max_phase, step_phase, enable_dual_amplitude_steps,
                                    start_time, experiment_duration):
    data = []
    ...
    for velocity in np.arange(min_velocity, max_velocity + step_velocity, step_velocity):
        for amplitude in np.arange(min_amplitude, max_amplitude + step_amplitude, step_amplitude):
            for phase in np.arange(min_phase, max_phase + step_phase, step_phase):
                data.append({"Time": current_time, "VelocityCommand": velocity, "AmplitudeCommand": amplitude, "PhaseCommand": phase})
                current_time += timedelta(seconds=time_step)

        if enable_dual_amplitude_steps:
            for amplitude in np.arange(max_amplitude, min_amplitude - step_amplitude, -step_amplitude):
                for phase in np.arange(min_phase, max_phase + step_phase, step_phase):
                    data.append({"Time": current_time, "VelocityCommand": velocity, "AmplitudeCommand": amplitude, "PhaseCommand": phase})
                    current_time += timedelta(seconds=time_step)
...
data = generate_thrust_vector_sequence(min_velocity, max_velocity, step_velocity,
                                       min_amplitude, max_amplitude, step_amplitude,
                                       min_phase, max_phase, step_phase, enable_dual_amplitude_steps,
                                       start_time, experiment_duration)
